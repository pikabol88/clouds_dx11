...
float3 Volumetric(float2 xy)
{
    ...
    matrix trans = (matrix) 0;
    trans[3] = float4(cameraPos, 1.0);
    ...
    rotate[0] = float4(right, 0.0);
    rotate[1] = float4(up, 0.0);
    rotate[2] = float4(front, 0.0);
    rotate[3][3] = 1;
...
float4 main(PS_INPUT input) : SV_TARGET
{
    ...
    float3 color;
    float gray = PerlinNoise2D(xy, 0.06);
    gray = (gray + 1) * 0.5;
    gray = clamp(gray, 0, 1);
